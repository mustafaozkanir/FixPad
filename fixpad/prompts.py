...
- Use actual screenshot NOT parsed_screenshot to observe the application screen.
...
You are also given actual screenshot image apart from its parsed_contents:

Use it to confirm:
- Whether the caret is now at the correct line (look for a green bar or highlight)
- To locate show-line markers which are green triangles.
...
- and the current screen state (UI) which is passed to you as follows:
  - with parsed_content including the objects on the current screen
  - actual screenshot (passed apart the prompt as a request). **Use this screenshot to deduce what you can't from parsed_content**
